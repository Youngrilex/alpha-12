...
const TableComponent = ({ darkMode }: { darkMode: boolean }) => {
  ...
  // Define styles for status badges
  ...
  return (
    <table className={`w-full text-left border-collapse mt-2 ${darkMode ? "bg-[#1F2937] text-white" : "bg-white text-gray-900"}`}>
      <thead>
        <tr className={`text-xs font-semibold h-12 ${darkMode ? "bg-[#6A6676] text-[#FCF7FF]" : "bg-[#F1F5F9] text-[#64748B]"}`}>
      ...
      <tbody>
        {events.map((event, index) => (
          <React.Fragment key={index}>
            ...
            >
              <td className="px-4 py-3 text-sm">
                <div className="flex gap-5 items-center">
                  ...
                  <span className={`${darkMode ? "text-[#FCF7FF]" : ""}`} onClick={handleOpenModal}>
                    {event.name}
                  </span>
                </div>
              </td>
            ...
            {expandedRows[index] && (
              <tr>
                <td colSpan={2} className={`px-4 py-3 ${darkMode ? "bg-[#383544] text-gray-300" : "bg-[#F1F5F9] text-gray-800"}`}>
      ...
      {/* Modal to display additional event details */}
